%\VignetteIndexEntry{Bioconductor LaTeX Style}
%\VignettePackage{BiocStyle}
%\VignetteEngine{knitr::knitr}

\documentclass{beamer}

\usetheme{Boadilla}
\setbeamertemplate{footline}
{
  \leavevmode%
  \hbox{%
  \begin{beamercolorbox}[wd=.5\paperwidth,ht=2.25ex,dp=1ex,right,rightskip=1em]{section in head/foot}%
    \usebeamerfont{subsection in head/foot}\hspace*{2ex}\insertshorttitle
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.4\paperwidth,ht=2.25ex,dp=1ex,left,leftskip=1em]{subsection in head/foot}%
    \usebeamerfont{section in head/foot}\insertsectionhead\hspace*{2ex}
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.1\paperwidth,ht=2.25ex,dp=1ex,right]{date in head/foot}%
    \insertframenumber{} / \inserttotalframenumber\hspace*{2ex}
  \end{beamercolorbox}}%
  \vskip0pt%
}

\AtBeginSection[]{
  \begin{frame}<beamer>
    \tableofcontents[currentsection]
  \end{frame}
}

\newcommand{\software}[1]{\textsl{#1}}
\newcommand\R{\software{R}}
%
\newcommand\Bioconductor{\software{Bioconductor}}
\newcommand{\Rpackage}[1]{\textsl{#1}}
\newcommand\Biocpkg[1]{%
  {\href{http://bioconductor.org/packages/#1}%
    {\Rpackage{#1}}}}
\newcommand\CRANpkg[1]{%
  {\href{http://cran.fhcrc.org/web/packages/#1/index.html}%
    {\Rpackage{#1}}}}
\newcommand{\Rcode}[1]{\texttt{#1}}
\newcommand{\Rfunction}[1]{\Rcode{#1}}
\newcommand{\Robject}[1]{\Rcode{#1}}
\newcommand{\Rclass}[1]{\textit{#1}}

\title{\R{} / \Bioconductor{} for 'Omics Analysis}
\author{Martin Morgan}
\institute[RPCI]{
  Roswell Park Cancer Institute\\
  Buffalo, NY, USA\\
  \href{mailto:martin.morgan@roswellpark.org}{martin.morgan@roswellpark.org}}
\date{30 January 2017}

\begin{document}

\maketitle

\begin{frame}{Introduction}
  \begin{columns}
  \column{.5\textwidth}
  \includegraphics[width=.8\textwidth]{bioconductor_logo_rgb.pdf}
  \bigskip\par
  \includegraphics[width=.15\textwidth]{Rlogo.png}
  \hspace{1em}
  \includegraphics[width=.3\textwidth]{nid61380-a1-rpci-cmyk-black.pdf}
  \bigskip\par
  \href{https://bioconductor.org}{https://bioconductor.org}\par
  \href{https://support.bioconductor.org}{https://support.bioconductor.org}
  \column{.5\textwidth}
  Analysis and comprehension of high-throughput genomic data.
  \begin{itemize}
  \item Started 2002
  \item 1295 packages -- developed by `us' and user-contributed.
  \end{itemize}
  Well-used and respected.
  \begin{itemize}
  \item 43k unique IP downloads / month.
  \item 17,000 PubMedCentral citations.
  \end{itemize}
  \end{columns}
\end{frame}

\section{About}

\begin{frame}{Scope}
  Based on the \R{} programming language.
  \begin{itemize}
  \item Intrinsically statistical nature of data.
  \item Flexiblity for new or customized types of analysis.
  \item `Old-school' scripts for reproducibility; modern graphical
    interfaces for easy use.
  \end{itemize}
  Domains of application.
  \begin{itemize}
  \item Sequencing: differential expression, ChIP-seq, variants,
    gene set enrichment, \ldots
  \item Microarrays: methylation, SNP, expression, copy number, \ldots
  \item Flow cytometry, proteomics, \ldots
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{\R{}: base packages}
<<r>>=
x <- rnorm(100)
y <- x + rnorm(100, sd=.5)
df <- data.frame(X=x, Y=y)
fit <- lm(Y ~ X, df)
anova(fit)
@
\end{frame}

\begin{frame}[fragile]{\R{}: contributed packages}
  \begin{columns}
    \column{.5\textwidth}
<<ggplot, fig.show="hide">>=
library(ggplot2)
ggplot(df, aes(x=x, y=y)) +
    geom_point() +
    stat_smooth(method="lm")
@
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{figure/ggplot-1}
  \end{columns}
\end{frame}

\begin{frame}{\Bioconductor{}}
  \begin{columns}
    \column{.4\textwidth}
    Learn \& use
    \begin{itemize}
    \item
      \textbf<1,2>{biocViews}\footnote[frame]{\url{https://bioconductor.org/packages/release}}
    \item \textbf<3>{Landing pages}\footnote[frame]{e.g.,
      \url{https://bioconductor.org/packages/edgeR}}
      \begin{itemize}
      \item Description
      \item Installation
      \item Documentation
      \end{itemize}
    \item \textbf<4-6>{Vignettes}\footnote[frame]{e.g.,
      \url{https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.pdf}}
    \item
      \textbf<7>{Workflows}\footnote[frame]{\url{http://bioconductor.org/help/workflows}},
      F1000 channel
    \end{itemize}
    \column{.6\textwidth}
    \includegraphics<1>[width=.6\textwidth]{biocViews.png}
    \includegraphics<2>[width=.9\textwidth]{ChIPSeq-views.png}
    \includegraphics<3>[width=.9\textwidth]{edgeR-landing-page.png}
    \includegraphics<4>[width=.8\textwidth]{DESeq-vignette-1.png}
    \includegraphics<5>[width=.8\textwidth]{DESeq-vignette-2.png}
    \includegraphics<6>[width=.8\textwidth]{DESeq-vignette-3.png}
    \includegraphics<7>[width=.9\textwidth]{Workflows.png}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{\Bioconductor{}}

  Input: description of experimental design and summary of read counts
  overlapping regions of interest.
  %%
<<bioconductor, eval=FALSE>>=
assay <- read.table("assay.tab")  # Plain text files
pdata <- read.table("pdata.tab")

library(DESeq2)
dds <- DESeqDataSetFromMatrix(assay, pdata, ~ cell + dex)
result(DESeq(dds))
@
%%
  Output: top table of differentially expressed genes, log fold
  change, adjusted $P$-value, etc.

\end{frame}

\section{'Omics workflows}

\begin{frame}{A typical work flow: RNA-seq}
  \begin{columns}
    \column{.5\textwidth}
    \begin{enumerate}
    \item Experimental design
    \item Wet-lab
    \item \textbf<1>{Sequencing; QC} -- FASTQ
      \only<1-2>{
      \item \textbf<2>{Alignment} -- BAM
      \item \textbf<2>{Data reduction} -- count tables
      }
      \only<3->{
      \item \textbf<3>{Psuedo-alignment} -- count tables
      }
    \item \textbf<4>{Statistical analysis}
    \item \textbf<4>{Comprehension}
    \end{enumerate}
    \column{.5\textwidth}
    \only<1-2>{
      \centering\includegraphics[width=\textwidth]{rna4.jpg}
      \medskip\par\url{http://bio.lundberg.gu.se/courses/vt13/rnaseq.html}
    }
    \only<3>{
      \par\noindent
      kallisto\footnote[frame]{https://pachterlab.github.io/kallisto/},
      salmon\footnote[frame]{http://salmon.readthedocs.io/}, \ldots
      \begin{itemize}
      \item \emph{Very} fast
      \item Very memory efficient
      \item Good enough for many applications
      \end{itemize}
      \Bioconductor
      \begin{itemize}
      \item \Biocpkg{tximport}
      \item \Biocpkg{limma} \texttt{voom()}
      \end{itemize}
    }
    \only<4>{
      \par\noindent
      \includegraphics[width=.2\textwidth]{Rlogo.png}
      \hspace{1em}
      \includegraphics[width=.6\textwidth]{bioconductor_logo_rgb.pdf}
      \bigskip\par
      \begin{itemize}
      \item \Biocpkg{DESeq2}, \Biocpkg{edgeR}
      \item Gene set / pathway analysis
      \item Annotation \& visualization
      \end{itemize}
    }
  \end{columns}
\end{frame}

\section{Lessons learned}

\begin{frame}[fragile]{Differential expression}
  \Biocpkg{limma}, \Biocpkg{edgeR}, \Biocpkg{DESeq2}
  %%
<<bioconductor-1, eval=FALSE>>=
library(DESeq2)
dds <- DESeqDataSetFromMatrix(assay, pdata, ~ cell + dex)
result(DESeq(dds))
@
%%
  \begin{itemize}
  \item Batch effects (e.g., surrogate variable analysis)
  \item Library size differences (robust normalization)
  \item Appropriate statistical model (negative binomial)
  \item Moderated, data-driven parameter estimates (shared design;
    small sample size)
  \item Multiple testing (independent hypothesis weighting)
  \end{itemize}
\end{frame}

\begin{frame}{Interoperability \& reproducibility: classes}
  \begin{columns}
    \column{.5\textwidth}
    \textbf<1>{\Biocpkg{GenomicRanges}}
    \begin{itemize}
    \item Genomic coordinates to represent data (e.g., aligned reads)
      and annotations (e.g., genes, binding sites).
    \item \Rcode{findOverlaps()} and friends.
    \end{itemize}
    \textbf<2>{\Biocpkg{SummarizedExperiment}}
    \begin{itemize}
    \item Coordinate `assay' data with row (feature) and column (sample)
      information.
    \end{itemize}
    \column{.5\textwidth}
    \includegraphics<1>[width=\textwidth]{GRanges.pdf}
    \includegraphics<2>[width=\textwidth]{SummarizedExperiment.pdf}
  \end{columns}
\end{frame}

\begin{frame}{Classic, tidy, rich: RNA-seq count data}
  Classic
  \begin{itemize}
  \item Sample x (phenotype + expression) Feature \Rcode{data.frame}
  \end{itemize}
  Tidy
  \begin{itemize}
  \item 'Melt' expression values to two long columns, replicated
    phenotype columns. End result: long data frame.
  \end{itemize}
  Rich, e.g., \Rcode{SummarizedExperiment}
  \begin{itemize}
  \item Phenotype and expression data manipulated in a coordinated
    fashion but stored separately.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{Classic, tidy, rich: RNA-seq count data}
  \begin{semiverbatim}
## Manipulate, e.g., mean expression of each gene

df0 <- data.frame(mean=colMeans(classic[, -(1:22)]))
df1 <- tidy %>% group_by(probeset) %>%
    summarize(mean=mean(exprs))
df2 <- data.frame(mean=rowMeans(assay(rich)))

## Visualize

ggplot(df1, aes(mean)) + geom_density()
  \end{semiverbatim}
\end{frame}

\begin{frame}{Classic, tidy, rich: RNA-seq count data}
  \begin{columns}[t]
    \column{.5\textwidth}
    Vocabulary
    \begin{itemize}
    \item Classic: extensive
    \item Tidy: restricted ‘endomorphisms’
    \item Rich: extensive, ‘meaningful’
    \end{itemize}
    Constraints (e.g., probes \& samples)
    \begin{itemize}
    \item Tidy: implicit
    \item Classic, Rich: explicit
    \end{itemize}
    Flexibility
    \begin{itemize}
    \item Classic, tidy: general-purpose
    \item Rich: specialized
    \end{itemize}

    \column{.5\textwidth}
    Programming contract
    \begin{itemize}
    \item   Classic, tidy: limited
    \item Rich: strict
    \end{itemize}
    Lessons learned / best practices
    \begin{itemize}
    \item Considerable value in semantically rich structures
    \item Endomorphism, simple vocabulary, consistent paradigm aid use
    \end{itemize}
  \end{columns}
\end{frame}

\section{Challenges}

\begin{frame}{Single-cell analysis}
  \begin{columns}
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{monocle-trajectory.png}
    \medskip\par Trapnel et al.\footnote[frame]{\url{http://bioconductor.org/packages/monocle}}
    \column{.5\textwidth}
    \begin{itemize}
    \item Large \& sparse
      \begin{itemize}
      \item Outlier detection
      \item Zero-inflated models
      \item E.g., \Biocpkg{MAST}
      \end{itemize}
    \item Challenging
      \begin{itemize}
      \item E.g., developmental trajectories
      \end{itemize}
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{Comprehension}
\begin{columns}
  \column{.5\textwidth}
  Gene set \& pathway analysis
  \begin{itemize}
  \item \Biocpkg{limma} \Rcode{fry()}; \Biocpkg{pathview};
    \textbf<1>{\Biocpkg{ReactomePA}}
  \end{itemize}
  Visualization
  \begin{itemize}
  \item \textbf<2>{\Biocpkg{Gviz}}, \textbf<3>{\Biocpkg{ComplexHeatmap}}, \ldots
  \end{itemize}
  Communication
  \begin{itemize}
  \item Reports; interactive apps
  \item \textbf<4>{Statistical nuance}, especially uncertainty, multiple testing
  \end{itemize}
  \column{.5\textwidth}
  \includegraphics<1>[width=\textwidth]{ReactomePA.png}
  \includegraphics<2>[width=.9\textwidth]{Gviz-0.png}
  \bigskip\par
  \includegraphics<2>[width=.9\textwidth]{Gviz.png}
  \includegraphics<3>[width=\textwidth]{ComplexHeatmap.png}
\end{columns}
\end{frame}

\begin{frame}{Multi-'omic integration}
  \begin{columns}[t]
    \column{.5\textwidth}
    Gene differential expression
    \begin{itemize}
    \item RNA-seq -- \Biocpkg{DESeq2}, \Biocpkg{edgeR},
      \Biocpkg{limma} \Rcode{voom()}
    \item Microarray -- \Biocpkg{limma}
    \item Single-cell -- \Biocpkg{scde}
    \end{itemize}
    Gene regulation
    \begin{itemize}
    \item ChIP-seq -- \Biocpkg{csaw}, \Biocpkg{DiffBind}
    \item Methylation arrays -- \Biocpkg{missMethyl}, \Biocpkg{minfi}
    \item Gene sets and pathways -- \Biocpkg{topGO}, \Biocpkg{limma},
      \Biocpkg{ReactomePA}
    \end{itemize}
    \column{.5\textwidth}
    Variants
    \begin{itemize}
    \item SNPs -- \Biocpkg{VariantAnnotation}, \Biocpkg{VariantFiltering}
    \item Copy number
    \item Structural -- \Biocpkg{InteractionSet}
    \end{itemize}
    Flow cytometry
    \begin{itemize}
    \item \Biocpkg{flowCore} \& 41 other packages
    \end{itemize}
    Proteomics
    \begin{itemize}
    \item \Biocpkg{mzR}, \Biocpkg{xcms}, and 90 other packages
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{Multi-'omic integration}
  \begin{columns}
    \column{.5\textwidth}
    \centering\includegraphics[width=.8\textwidth]{copy_number_QC_2.pdf}
    \column{.5\textwidth}
    \Biocpkg{MultiAssayExperiment}
    \begin{itemize}
    \item Easily manage multiple assays on overlapping samples
    \end{itemize}
    \Biocpkg{ExperimentHub}
    \begin{itemize}
    \item Curated, summarized, large-scale experiment data (e.g., GEO
      RNA-Seq; HMP, TCGA) for incorporation in local analysis
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{Big data}
  \begin{columns}
    \column{.5\textwidth}
    Key strategies
    \begin{itemize}
    \item Efficient \R{} code
    \item Restriction to data of interest
    \item Chunk-wise iteration through large data
    \end{itemize}
    \column{.5\textwidth}
    \Biocpkg{GenomicFiles}
    \begin{itemize}
    \item Management of file collections, e.g., VCF, BAM, BED.
    \end{itemize}
    \Biocpkg{BiocParallel}
    \begin{itemize}
    \item Parallel evaluation on cores, clusters, clouds.
    \end{itemize}
    \Biocpkg{HDF5Array}
    \begin{itemize}
    \item On-disk storage.
    \item Delayed evaluation.
    \item Incorporates into \Rcode{SummarizedExperiment}.
    \end{itemize}
  \end{columns}
\end{frame}

\section{Opportunities}

\begin{frame}{Install, learn, use, develop}
  \begin{columns}
    \column{.6\textwidth}
    \includegraphics[width=\textwidth]{install-learn-use-devel.png}
    \column{.4\textwidth}
    Install\footnote[frame]{\url{https://bioconductor.org}}
    \begin{itemize}
    \item \R{}, \software{RStudio},
      \Bioconductor{}
    \end{itemize}
    Learn
    \begin{itemize}
    \item Courses, vignettes, workflows
    \end{itemize}
    Use
    \begin{itemize}
    \item Vignettes, manuals, support
      site\footnote[frame]{\url{https://support.bioconductor.org}}
    \end{itemize}
    Develop
  \end{columns}
\end{frame}

\begin{frame}{From student to developer}
  A common transition
  \begin{itemize}
  \item Naive users become proficient while developing domain
    expertise that they share with others in their lab or more broadly
  \item Share via packages
  \item Really easy!
  \end{itemize}
  Best practices
  \begin{itemize}
  \item \CRANpkg{devtools} \Rcode{create()}, \Rcode{build()}, \Rcode{check()}, \Rcode{install()}
  \item Version control -- github
  \item Unit tests, e.g., using \CRANpkg{testthat}
  \item `Continuous integration'
  \end{itemize}
\end{frame}

\section*{Acknowledgments}

\begin{frame}{Core team jobs!}
  \begin{itemize}
  \item Scientific Programmer / Analyst -- core packages; \emph{R} and
    \emph{C} algorithms.
  \item Senior Programmer / Analyst -- system / cloud management.
  \item \url{https://support.bioconductor.org/p/91548/}
  \end{itemize}
\end{frame}

\begin{frame}{Acknowledgments}
  Core team (current \& recent): Yubo Cheng, Valerie Obenchain,
  Herv\'e Pag\`es, Marcel Ramos, Lori Shepherd, Dan Tenenbaum, Nitesh
  Turaga, Greg Wargula.
  \medskip\par
  Technical advisory board: Vincent Carey, Kasper Hansen, Wolfgang
  Huber, Robert Gentleman, Rafael Irizzary, Levi Waldron, Michael
  Lawrence, Sean Davis, Aedin Culhane
  \medskip\par
  Scientific advisory board: Simon Tavare (CRUK), Paul Flicek
  (EMBL/EBI), Simon Urbanek (AT\&T), Vincent Carey (Brigham \&
  Women's), Wolfgang Huber (EBI), Rafael Irizzary (Dana Farber),
  Robert Gentleman (23andMe)
  \medskip\par
  Research reported in this presentation was supported by the National
  Human Genome Research Institute and the National Cancer Institute of
  the National Institutes of Health under award numbers U41HG004059
  and U24CA180996. The content is solely the responsibility of the
  authors and does not necessarily represent the official views of the
  National Institutes of Health.

\end{frame}

\end{document}
