...
\begin{frame}{Pre-processing, alignment}  
  ...
  \begin{itemize}
  \item Full alignment to BAM files, summarizing gene or transcript
    abundance, e.g., \software{Bowtie} / \software{tophat} /
    \software{cufflinks}; \software{RSEM}; \Biocpkg{Rsubread}
  \item Summarize to gene-level count tables or estimates of abundance
  \item \emph{Counts} are important: information about statistical
    uncertainty of estimate
  \end{itemize}
  Alignment \& summary (contemporary)
  \begin{itemize}
  \item Approximate alignment directly to count tables of transcripts
    or genes, e.g.,
