...
\begin{frame}{State of the project}
  \begin{columns}
    \column{.5\textwidth}
    \only<1>{
      \includegraphics[width=\textwidth]{packages-per-release.png}
    }
    \only<2>{
    ...
    \only<4>{
      \includegraphics[width=\textwidth]{build-report-overview.png}
      \par
      \includegraphics[width=\textwidth]{build-report-detail.png}
    }
    \only<5>{
      \begin{center}
...
\begin{frame}{Recent developments}
  ...
  \item New package submission
    \begin{itemize}
    \item As github issues
    \item Public; review participation welcome
    \end{itemize}
  \item \Biocpkg{ExperimentHub} and \Biocpkg{AnnotationHub}
    \begin{itemize}
    \item Similar to `Annotation' and `Experiment' data repositories
    \item \Biocpkg{ExperimentHub} often used as the 'data store' for
      experiment data packages, e.g., \Biocpkg{alpineData}.
    \end{itemize}
...
> library(h5vcData)
> h5file <- system.file("extdata", "example.tally.hfs5",
                        package="h5vcData")
> cov0 <- HDF5Array(h5file, "/ExampleStudy/16/Coverages")
> pcov <- t(drop(cov0[ , 1, ]))  # coverage on plus strand
> mcov <- t(drop(cov0[ , 2, ]))  # coverage on minus strand

> library(SummarizedExperiment)
> SummarizedExperiment(list(pcov=pcov, mcov=mcov))
...
> library(Organism.dplyr)
    ## biocLite("Biocondctor/Organism.dplyr")
...
> left_join(ids, tbl(src, "ranges_tx")) %>% GRanges
...
\begin{frame}{Programming best practices}
  ...
  \item Reuse \& interoperability
    \begin{itemize}
    \item \Biocpkg{GenomicRanges} and \Biocpkg{SummarizedExperiment}
    \item \Biocpkg{rtracklayer}\Rcode{::import()} for BED, WIG, GTF,
      GFF, etc.
    \end{itemize}
...
\begin{frame}{Acknowledgments}
  Core team: Yubo Cheng, Valerie Obenchain, Herv\'e Pag\`es, Marcel
  Ramos, Lori Shepherd, Nitesh Turaga, Greg Wargula.
  \medskip\par
