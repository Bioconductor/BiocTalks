...
\begin{frame}{Sponsors}
  \begin{center}
    \includegraphics[width=.4\textwidth]{rstudio.png}\hspace{2em}
    \includegraphics[width=.4\textwidth]{genentech.png}
    \bigskip\par
    \includegraphics[width=.4\textwidth]{sevenbridges.png}\hspace{2em}
    \includegraphics[width=.4\textwidth]{astrazeneca.PNG}
    \bigskip\par
    \includegraphics[width=.4\textwidth]{dana-farber.jpg}\hspace{2em}
    \includegraphics[width=.4\textwidth]{rpci-logo_1.png}
  \end{center}
\end{frame}
...
\begin{frame}{Logistics}
  Posters
  \begin{itemize}
  \item Please leave at the registration desk, today if possible,
    Thursday morning at the latest.
  \end{itemize}
  Conference Amazon Machine Instances
  \begin{itemize}
  \item Start yours today at \url{courses.bioconductor.org}
  \item Username: \ldots; password: \ldots
  \item `This site can't be reached...' ?? Reload page (AMI is still
    spinning up)
  \item `We're sorry, but something went wrong' ?? Re-submit request
    for AMI or notify Lori (Amazon ran out of cloud!)
  \end{itemize}
\end{frame}
